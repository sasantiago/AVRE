...
describe('UsersService', () => {
  ...
  describe('createUser', () => {
    it('rechaza si el email ya existe', async () => {
      ...
      await expect(
        service.createUser(actor, { email: 'x@e2e.test', password: 'x', fullName: 'X', role: Role.ADVISOR }),
  ...
  describe('getAdvisorClientDetail', () => {
    it('lanza NotFoundException si el cliente no pertenece a ese asesor', async () => {
      ...
      await expect(service.getAdvisorClientDetail('adv-1', 'client-de-otro-asesor')).rejects.toBeInstanceOf(
        NotFoundException,
      );
    });
